...
\begin{align*}
    ...
    \text{(for DC)} &\approx -\frac{R_1}{R_2} = -10 \frac{V}{V} \\= 20 \space \text{dB} \\
\end{align*}
...
Na wejście układu podano sygnał prostokątny o amplitudzie $100mV_{pp}$ o częstotliwości:
\vspace{15pt}

\noindent a) niższej niż częstotliwość graniczna filtra: 10 Hz
\begin{center}
    \centering
    \includegraphics[width=0.9\linewidth]{lab_1-2/02_odwracajacy/10Hz.png}
\end{center}

\noindent b) nieco wyższej niż częstotliwość graniczna filtra: 20 Hz
\begin{center}
    \centering
    \includegraphics[width=0.9\linewidth]{lab_1-2/02_odwracajacy/20Hz.png}
\end{center}


\noindent c) dużo większej niż częstotliwość filtra: 100 Hz
\begin{center}
    \centering
    \includegraphics[width=0.9\linewidth]{lab_1-2/02_odwracajacy/100Hz.png}
\end{center}

\noindent Układ całkujący wykorzystuje fakt, iż dla stałych czasowych $\tau >> \tau_{RC}$ ładowanie oraz rozładowywanie kondensatora przypomina całkowanie danego przebiegu. W miarę zmniejszania się częstotliwości sygnału do częstotliwości granicznej (danej przez R orac C wpięte w pętlę sprzężenia zwrotnego) widać iż podawany na wejście przebieg prostokątny przypomina ładowanie oraz rozładowywanie kondensatora, zamiast matematyczną operację całkowania.

\newcolumn 
\subsection{Wnioski z ćwiczenia}

\subsubsection{Wtórnik napięcia}

W pierwszej części laboratorium zweryfikowano teoretyczne rozważania, dotyczące układu wtórnika, przeprowadzono analizę charakterystyki przejściowej (DC), częstotliwościowej (AC), dokonano pomiarów parametru "Slew Rate" oraz zmierzono wpływ napięcia zasilania na sygnał - zaobserwowano efekt inwersji związany z przekroczeniem limitu napięcia typu common na wejściach.

\vspace{5pt}

Nasze rozważania teoretyczne/symulacje zgadzają się z wynikami uzyskanymi podczas pomiarów. Symulacje wykonane zostały w programie LTspice po odpowiednim skonfigurowaniu modelu \textit{UniversalOpamp2} aby zgadzał się z parametrami wzmacniacza TL082 z noty katalogowej.

\subsubsection{Wzmacniacz nieodwracający}

\noindent W tej części ćwiczenia przeprowadzono analizę teoretyczną dla układu wzmacniacza nieodwracającego o wzmocnieniu 100 (40dB). Wyznaczono charakterystyki DC oraz AC. Zaobserwowano wymianę wzmocnienia an pasmo.

\subsubsection{Filtr dolnoprzepustowy oparty o wzmacniacz
odwracający}

\noindent W ostatniej części ćwiczenia przeprowadzono analizę teoretyczną dla filtru dolnoprzepustowego I-rzędu o wzmocnieniu 10 oraz częstotliwości granicznej ~18Hz. Wyznaczono charakterystyki DC oraz AC. Zauważono, że układ całkuje przebiegi wejściowe dla częstotliwości znacznie wyższych od częstotliwości granicznej filtru.
